...
describe("external adapter smoke", () => {
  ...
  it("installs, exposes, and uses a local external adapter package", async () => {
    ...
    const detect = await request(app).get("/api/companies/company-1/adapters/external_smoke/detect-model");
    expect(detect.status, JSON.stringify(detect.body)).toBe(200);
    expect(detect.body).toMatchObject({
      model: "smoke-model",
