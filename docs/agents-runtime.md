...
- `hermes_local`: runs your local Hermes CLI (`@henkey/hermes-paperclip-adapter` or a local package path)
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-paperclip-adapter`)

The adapter manager shows whether external adapters are loaded and which optional contracts are present: config schema, UI parser, model detection, session management, and lifecycle hooks. If an installed adapter is not loaded, use reload/reinstall from the manager and inspect server logs for the package import error.
